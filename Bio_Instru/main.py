...
def segment_dataset(filepath, window_length_0=200, classes=None):
    ...
    for i in fileNames:
        ...
        if np.in1d(int(metaData[0]), classes):
          data_read_ch0 = np.loadtxt(filepath+i, delimiter=',')[6]  # Choosing channel 6 as first channel for this exercise
          data_read_ch1 = np.loadtxt(filepath+i, delimiter=',')[17] # Choosing channel 17 as second channel for this exercise
          ...
          data_windows_ch1 = [data_read_ch1[w * window_length_0:w * window_length_0 + window_length_0] for w in range(n_window)]
